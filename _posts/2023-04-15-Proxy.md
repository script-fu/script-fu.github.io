...
To restore proxies, select them, and run the plugin again.

Designed to work with groups that have a normal mode of compositing, however if
like me you also create composite inking groups where the mode is multiply you can still
use the proxy plugin. However put a parasite on the image called "ink". This can be
done with <https://script-fu.github.io/2023/05/16/AttachImageParasites.html>.
Now when the placeholder is made it has a multiply mode and looks as it should.
...
An additional plug-in, [proxy-restore-all](https://raw.githubusercontent.com/script-fu/script-fu.github.io/main/plug-ins/proxy-restore-all/proxy-restore-all.scm), can quickly restore the file to having no proxies.
